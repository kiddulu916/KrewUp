...
import { useExperiencePhotos } from '../hooks/use-experience-photos';
import {
  uploadExperiencePhoto,
  deleteExperiencePhoto,
  reorderExperiencePhotos,
} from '../actions/experience-photo-actions';
import type { ExperiencePhoto } from '../types';

type ExperiencePhotoManagerProps = {
  experienceId: string;
  profile: Profile;
};

/**
 * Sortable experience photo item for drag-and-drop
 */
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-square">
        <Image
          src={photo.image_url}
          alt={`Project photo ${photo.display_order + 1}`}
          fill
...
/**
 * ExperiencePhotoManager Component
 *
 * Manages project photos with upload, delete, and reorder functionality.
 ...
 */
export function ExperiencePhotoManager({ experienceId, profile }: ExperiencePhotoManagerProps) {
  const { data: photos = [], isLoading, error } = useExperiencePhotos(experienceId);
  ...
  const canUpload = isPro || photos.length < 5;
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await uploadExperiencePhoto(formData, experienceId);

      if (result.success) {
        toast.success('Photo uploaded successfully');
        // Refetch experience photos
        queryClient.invalidateQueries({ queryKey: ['experience-photos', experienceId] });
      } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const result = await deleteExperiencePhoto(deleteConfirmId);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const previousPhotos = queryClient.getQueryData<ExperiencePhoto[]>([
      'experience-photos',
      experienceId,
    ]);
    ...
    queryClient.setQueryData(['experience-photos', experienceId], newPhotos);

    setIsReordering(true);

    try {
      const photoIds = newPhotos.map((photo) => photo.id);
      const result = await reorderExperiencePhotos(photoIds);
      ...
      if (result.success) {
        ...
        if (previousPhotos) {
          queryClient.setQueryData(['experience-photos', experienceId], previousPhotos);
        }
      }
    } catch {
  ...
  const handleUploadClick = () => {
    if (!canUpload) {
      toast.warning(
        'Free users can upload maximum 5 photos per project. Upgrade to Pro for unlimited photos.'
      );
      return;
    }
  ...
  if (error) {
    return (
      <div className="bg-red-50 border-2 border-red-200 rounded-lg p-6">
        <p className="text-red-800">Failed to load photos. Please try again.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-gray-600">
            {isPro ? (
              'Add unlimited photos to showcase this project'
            ) : (
              <>
                {photos.length} of 5 photos used.{' '}
        ...
        <div>
          ...
          <Button
            ...
            disabled={isUploading || (!isPro && photos.length >= 5)}
          >
      ...
      {photos.length === 0 ? (
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
          ...
          <h4 className="text-lg font-semibold text-gray-900 mb-2">No project photos yet</h4>
          <p className="text-gray-600 mb-4">
            Upload photos to showcase your work on this project
          </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {photos.map((photo) => (
                  <SortablePhotoItem
                    ...
                    isDeleting={isDeletingId === photo.id}
                  />
                ))}
              </div>
            </SortableContext>
